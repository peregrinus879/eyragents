...
Long work keeps a live plan file, as global guidance's Continuity rule describes. `ship` shows every card of a round in the chat first, then H approves each `git commit` at the native prompt. Publication waits for H's go, then each push prompts the same way. Host-local authentication supplies capability, not approval; [setup](docs/setup.md#github-access) owns standalone onboarding.
