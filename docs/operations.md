...
Failed CI after an approved publication follows the [publish skill's CI rerun procedure](../agents/.agents/skills/publish/SKILL.md#ci-reruns): verify run/job ownership and the full SHA, inspect the failure, establish a retryable cause, and retry the smallest affected job scope. `gh run rerun` asks in both tools; state the run, job and cause, and H approves at the prompt.
