...
`publish-bind` prints the reviewed push argv, exact `publish-apply ID` invocation, repository and record-root context. Present the complete packet and obtain H's contemporaneous exact-target approval before execution. Use the native tool's repository context and the same `EYRAGENTS_RECORD_ROOT`; the helper accepts only ID. Obtain any required native execution authority first. Codex prepares the review but hands execution and verification to a separately launched Claude Code, OpenCode or Hermes primary. The receiving primary verifies the saved context and obtains fresh approval; follow the [publish skill](../agents/.agents/skills/publish/SKILL.md). [Access policy](access.md#codex) explains why the protected-file and network restrictions remain.
