...
The default profile uses **Astra (`gpt-6-astra`) through ChatGPT/Codex OAuth at `xhigh`**, with native `coding_context: auto`, local terminal execution, smart command approvals, memory, skill learning, curator and delegation. The harness retains Hermes's normal toolset. Resume may restore a recorded working directory; use Hermes's `--in` option directly when you need to pin it.
...
An implementation request includes in-scope work under `~/Projects/scratch`; existing work there remains preserved project data. Use a unique owned child for disposable tests. OpenCode/Hermes require supported ownership, canonical-path, mount and target metadata; an absent, linked or unsafe root receives no new automatic grant. Claude uses a scoped Edit allow, and Codex declares scratch as a profile workspace root with its existing metadata/credential restrictions. [The matrix](access.md#workspace-and-home) distinguishes these controls.

After restarting the affected clients, verify ordinary file creation/update in an owned scratch child, protected-path refusal using synthetic fixtures, and the unchanged behavior of an external target outside scratch. Use Add/Delete for OpenCode scratch moves. Do not repair ownership/links or delete neighboring work merely to obtain a passing probe. Codex's upstream startup limitation keeps its live acceptance separate.
...
Before a fresh-client probe, `HERMES_PYTHON -I -B tests/hermes.py --metadata-preflight` checks safe boolean/count summaries of actual home, protected-path and mount metadata and prospective scratch eligibility. It reads no credential contents and creates no target. This is host-layout evidence, not permission dispatch. The installed-runtime suite also checks both composed search guards, native write denials and native memory/skill/curator persistence.
...
- Optional installed-launcher integration: `python3 tests/hermes-live.py hdw --hdw /absolute/path/to/installed/hdw` starts a uniquely named Herdr server with fake HOME/XDG/config and pane PTYs. A recording Hermes stub verifies `hdw ha -c` arguments, the new three-pane workspace and physical cwd without selecting an existing conversation. Real-client resume is checked separately with an explicitly owned session ID. Supply the launcher explicitly; standalone harness verification needs no host-repository checkout.
