...
Quarry is H's shared reference-clone root, not disposable scratch. [Eyrsync](../.agents/skills/eyrsync/SKILL.md#reference-lifecycle) owns H-authorized task-required refreshes of this repository's existing declared clones, including verified same-project canonical URL reconciliation using pinned GitHub IDs. The standalone updater previews its own manifest, preserves local work, refs, tags and explicit push URLs, and refuses unsafe or uncertain inputs. These shell operations are distinct from native file-write cells: client command controls and Codex's external-write/network restrictions remain. New clones, different sources, arbitrary reference-source edits, force updates and disposal require their own authority; no location grant bypasses those boundaries.
...
Startup defaults are owned by [the mise fragment](../opencode/.config/mise/conf.d/eyragents-opencode.toml), applied through mise activation, shims or `mise exec`. It preserves explicit caller values and needs no host-dotfiles exports. OpenCode 1.18.30 reads these runtime flags before loading plugins, so a plugin that sets them later would not establish the same behavior. The existing external-skill-disable and other permission restrictions remain authoritative.
...
| Quarry maintenance | Task-required refreshes and pinned-ID same-project URL reconciliation for this repository's existing references | Own-manifest preservation-first updater; Claude classifier exception; native command/network restrictions remain | Authorized workflow; enforcement differs |
