...
An Ask is a native prompt: the agent states what the command does and H selects. The [ship skill](../agents/.agents/skills/ship/SKILL.md) shows every card of a round before its first prompt, then each commit or push prompts on its own, and publication also waits for H's go.
