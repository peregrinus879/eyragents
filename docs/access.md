...
An Ask is a native prompt: the agent states what the command does and H selects. The [ship skill](../agents/.agents/skills/ship/SKILL.md) shows every card of a round, then one command commits or pushes the round at a single prompt, and publication also waits for H's go.
