...
H approves each commit and each push at the tool's native permission prompt. The ship skill shows a card before each, so the prompt is an informed decision about the displayed command, and publication additionally waits for H's go. Git history is the record; no receipt, binding or shell gate stands between the card and the prompt. Destructive or hard-to-reverse commands keep their own prompts.
