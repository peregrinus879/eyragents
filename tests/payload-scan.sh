#!/usr/bin/env bash
# Behavioral checks for the payload scanner that commit and publish review use:
# outbound requests and artifacts, replies, diffs, the shared path corpus, and
# the repository's own tracked content. Fixtures that must look like
# credentials are assembled at runtime so the repository itself stays scannable.
set -euo pipefail

[[ $# == 0 ]] || { printf 'usage: payload-scan.sh\n' >&2; exit 64; }
...
mkdir -p "$HOMEBOX" "$TMP/art" "$WORK/deployed"
chmod 700 "$TMP"
chmod 755 "$TMP/art"
cp -- "$SCANNER" "$WORK/deployed/spar-payload-scan"
...
python3 - "$WORK/deployed/spar-payload-scan" "$ROOT/tests/safety-paths.json" "$TMP" <<'PY'
import json
from pathlib import Path
import runpy
import subprocess
...
printf 'ok: payload scanner accepts ordinary material and rejects credential-shaped content\n'
