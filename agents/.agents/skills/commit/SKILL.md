...
Enter when H has clearly directed commit preparation, committing or publication, including an existing `Commit and resume` direction. Otherwise use [develop's completion handoff](../develop/SKILL.md#finish-and-handoff); a vague acknowledgment of implementation is not preparation authority. Do not repeat the preparation question per repository or after resume when direction is already clear.

Reuse the existing [workstream state](../develop/references/workstream.md), atomic map and evidence. A ready checkpoint does not approve a commit or push. Return implementation defects to targeted development and verification; exact candidate and publication approvals remain mandatory.
...
1. Classify untracked files (`git ls-files --others --exclude-standard`) as intended new files or session scratch, and account separately for ignored workstream artifacts and receipts. Follow develop's [workstream lifecycle](../develop/references/workstream.md) for owned notes/evidence; preserve user/unknown files. Receipts use the separate close-out procedure below, not a blanket scratch sweep or permanent archive.
...
3. Run or reuse gates under the shared [verification contract](../develop/references/verification.md), including exact-tested-state and host requirements. If the tested worktree is proved to match the candidate and all relevant inputs/context are accounted for, reuse that evidence; staging alone does not require another snapshot or gate run. A receipt or fresh unverified brief is not gate evidence. Report failed/skipped checks explicitly.
...
3. Ensure gates cover the candidate's exact contents. A different working tree is not a superset or attestation of the index; validate a differing staged snapshot in a disposable context without deploying it, or defer the mixed candidate. The receipt and original gate/review evidence own the technical detail. Keep only the candidate ID, evidence pointers and next action in the checkpoint; do not duplicate receipt fields, hash inventories or passing logs. Technical records remain available through dependent steps and never attest H's approval.
...
2. Report the hash and title. `Commit and resume` continues the request's authorized work, preparing and approving remaining commits individually. When none remains, honor an explicit local-only endpoint through Receipt Close-Out; otherwise load `publish` and present one consolidated summary of the fixed reviewed bindings. Multiple separate commits on one branch can share its publication binding; their history and individual diffs remain intact. H's separate set approval permits one `publish-apply ID` attempt per listed binding, executed and verified individually in order. Present publication earlier only when a later step depends on it having landed. `Commit and pause` stops for discussion.
...
**Local-only completion:** when H explicitly ends the work after local commits and no approval, publication, CI or recovery still needs the selected candidate records, preview `commit-candidate --close ID... --local-only --dry-run`, then apply the same selection with `--local-only`. The helper validates each actual committed object against its receipt and refuses publication records in the selection; the primary checks remaining workflow dependencies. `LOCAL_ONLY` records local completion without claiming publication. Ready, active, ambiguous, corrupt and foreign-worktree candidates remain protected. Recovery requires the same flag. This mode cannot be combined with `--accept-unverified` and never requires creating a fictitious publication binding.
