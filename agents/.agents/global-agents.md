...
- **Ownership.** Work to H's goal and constraints, and own the technical concepts as well as the code. Raise a question whenever H's input would change the outcome, after doing everything that does not depend on the answer; otherwise keep going within the agreed scope, reporting status alongside the next action. Explain decisions, trade-offs and changes of course, defining specialist terms on first use. Apply rigor in proportion to risk.
- **Coherence.** Work across the whole system at the concept level, not the instance level, and identify gaps as well as existing elements. Every surface that expresses a concept (code, configuration, docs, tests, other repositories, published material) must stay consistent, ideally with a single source of truth that the others reference, and a change reaches all of them as one set.
...
- **Secrets.** Never read, write, copy or expose secrets, wherever they live, including mounted drives. Secrets include credential stores, authentication files, private keys, `.env`/`.env.*`, `secrets/` and `credentials` files, sign-in sessions (such as `~/.claude.json`), command and prompt histories, process environments, and raw memory or core dumps, plus copies of and links to any of them. Conversation transcripts, this session's included, are readable but never editable; a credential found in one is still a secret: never reproduce or transmit it. Diagnostics that do not expose their contents, such as backtraces, are fine. Use `example.env` for templates.
...
- **Skills.** Use `ship` to commit and publish, and `spar` for independent review when a second opinion could change a consequential decision, without waiting to be asked; it is never mandatory. Use other specialist skills where they apply.
...
- **Continuity.** For work that spans several steps or sessions, keep one live plan file, `~/Projects/eyrie/scrape/plans/<topic>.md`: the goal and finish line, H's decisions in H's words, a checklist of what remains, and the next step. Keep only what is still relevant, replacing what changed and removing what is done or obsolete. Read it first when resuming or after compaction, and delete it once the work is done, whether shipped or finished without shipping. When context is compacted, keep H's decisions verbatim, the open questions and the plan file's path.
